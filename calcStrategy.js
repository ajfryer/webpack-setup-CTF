/*
*
* Calculates the CryptoTrendFollowing.com single market trading strategy.
* Returns the current position size as well as backtested equity curves.
*
*/
...
function calcTotalReturns(returns,lookback) {
  ...
  for (let i = lookback-1; i < returns.length; i++) {
    ...
    totalReturns.push(totalReturns[len-1] + returns[i]*totalReturns[len-1]);
  }
  return totalReturns;
}
...
//
function calcRangeScalars(volatilityScaledTotalReturns, ranges, allowShorts, maxLeverage) {
  const rangeScalars = [];
...
//
function calcStrategy(dates, closes, lookback, targetVolatility, maxLeverage, allowShorts) {
  //
  ...
  //
  const volatilityScalars = calcVolatilityScalars(volatilities, targetVolatility)
  //console.log('volatilityScalars', volatilityScalars);
  //
  const volatilityScaledReturns = calcVolatilityScaledReturns(returns, volatilityScalars, lookback)
  //console.log('volatilityScaledReturns', volatilityScaledReturns);
  //
  const volatilityScaledTotalReturns = calcVolatilityScaledTotalReturns(volatilityScaledReturns)
  ...
  //
  const rangeScalars = calcRangeScalars(volatilityScaledTotalReturns, ranges, allowShorts, maxLeverage)
  //console.log('rangeScalars', rangeScalars);
  //
  const rangeScaledReturns = calcRangeScaledReturns(volatilityScaledReturns, rangeScalars)
  //console.log('rangeScaledReturns', rangeScaledReturns);
  //
  const rangeScaledTotalReturns = calcRangeScaledTotalReturns(rangeScaledReturns)
  //console.log('rangeScaledTotalReturns', rangeScaledTotalReturns);
  //
  const position = volatilityScalars[volatilityScalars.length-1] * rangeScalars[rangeScalars.length-1];
